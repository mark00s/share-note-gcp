            ...
            <form [formGroup]="noteForm" (ngSubmit)="onSubmit()" class="space-y-5">
              <div>
                ...
                <textarea
                  ...
                  rows="5"
                  placeholder="Write down your secret..."
                  class="w-full p-3 bg-gray-900 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                ></textarea>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label class="block text-gray-400 text-sm font-bold mb-2"
                    >Password (Encryption Key)</label
                  >
                  <input
              ...
              @if (errorMessage) {
                <div
                  class="bg-red-900/50 border border-red-500 text-red-200 p-3 rounded-lg text-sm"
                >
                  {{ errorMessage }}
                </div>
              }
        ...
        @if (generatedLink) {
          <div class="text-center">
            ...
            <p class="text-gray-400 mb-6">
              Share the following URL with recipient. Remember to provide password using different
              channel than the URL
            </p>
            <div
              class="flex items-center space-x-2 bg-gray-900 p-2 rounded-lg border border-gray-600 mb-6"
            >
              <input
                type="text"
                readonly
                [value]="generatedLink"
                class="flex-1 bg-transparent text-gray-300 outline-none px-2 text-sm"
              />
              <button
                (click)="copyLink()"
                class="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded transition-colors text-sm"
              >
                Copy
              </button>
            </div>
            <button (click)="resetForm()" class="text-blue-400 hover:text-blue-300 font-bold">
              Create another one
            </button>
          </div>
        }
